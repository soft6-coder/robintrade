...
getUserXhr.onreadystatechange = function() {
	if (this.status == 200 && this.readyState == 4) {
		...
		if (response.accountNonLocked == false) {
			...
			document.getElementById("firstname-confirm").innerText = firstName;
...
document.body.addEventListener("click", function(e) {
	...
	if (targetId == "start-conversation") {
		...
		document.getElementById("refer-modal").style.display = "none";
		confirmInvestment();
	} else if (targetId == "confirm-investment") {
...
function startInvestment() {
	let investment = {
		account: { accountId: userDetail.account.accountId },
		currency: { crypto: cryptoValues[cryptoChangeCount].name },
...
function getAccount() {
	...
	if (account.accountBalance >= 500) {
		document
			.getElementById("refer-a-friend")
			.classList.replace("green-background-inactive", "green-background");
		document
			.getElementById("refer-a-friend-mobile")
			.classList.replace("green-background-inactive", "green-background");
	}
	let spinner = document.getElementById("dashboard-spinner");
	spinner.className = spinner.className.replace("opacity-1", "opacity-2");
		...
		if (this.readyState == 4 && this.status == 200) {
			...
			if (response == null) {
				...
				if (days == 2) {
					totalTime = 48;
					expectedAmount =
						response.investedAmount * 10 - response.investedAmount;
				} else {
					totalTime = 96;
					expectedAmount =
						response.investedAmount * 20 - response.investedAmount;
				}
