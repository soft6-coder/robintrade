...
document.body.addEventListener("click", function(e) {
	...
	if (targetId == "start-conversation") {
		...
		prepareForSending();
	} else if (targetId == "open-fund-modal") {
	...
	cryptoUpdateXhr.onreadystatechange = function() {
		...
		if (this.readyState == 4 && this.status == 200) {
			...
			response.forEach(function(crypto) {
				let sortedCrypto = { name: crypto.name, symbol: crypto.symbol, price: crypto.current_price, image: crypto.image }
				cryptoUpdate.push(sortedCrypto);
			})
			cryptoUpdate.unshift({ name: 'Dollar', symbol: 'usd', price: 1, image: '/images/dollar.png' });
...
function getAccount() {
	...
	document.getElementById('available-to-withdraw').innerText = numberWithCommas(account.accountBalance.toFixed(1));
	document.getElementById('account-balance').innerText = numberWithCommas(account.accountBalance.toFixed(1));
	let spinner = document.getElementById("dashboard-spinner");
	spinner.className = spinner.className.replace("opacity-1", "opacity-2");
	document.getElementById("dashboard-container").style.display = "block";
	setTimeout(function() {
	...
	investmentXhr.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) {
			...
			if (response == null) {
				...
				document.getElementById('accrued-interest').textContent = (0).toFixed(1);
...
function setUserDetailsSetting(userDetails) {
	...
	accountType.forEach(function(accType) {
		...
		accountTypeSel.innerHTML += bindSelect(accType.accountTypeId, accType.accountType, selected)
	})
